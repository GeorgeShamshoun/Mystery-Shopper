...
import com.example.mysteryshopper.databinding.FragmentQuestions5Binding

class QuestionsFragment5 : Fragment() ,OnSelectedClick , OnPreviewClick{
    private var binding: FragmentQuestions5Binding? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentQuestions5Binding.inflate(inflater, container, false)
        return binding!!.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.txtQ5?.text =  MyExData().SetQuestions().get(questionId-1).QuestionText
        binding?.txtOtherQ5?.text =  MyExData().SetQuestions().get(questionId-1).QuestionOtherText
        ...
        binding?.rvQ5?.adapter = radioButtonRecyclerViewAdapter
       ...
       val attachedFileRecyclerViewAdapter = AttachedFileRecyclerViewAdapter()
        ...
        binding?.rvQ5Upload?.adapter = attachedFileRecyclerViewAdapter
        attachedFileRecyclerViewAdapter.onPreviewClick = this
    }

    override fun SelectClick(n: Int,d:Int,t : Boolean) {
        if (n == d){
            binding!!.txtOtherQ5.visibility = View.VISIBLE
            binding!!.edtAnswerQ5.visibility = View.VISIBLE
        }else{
            binding!!.txtOtherQ5.visibility = View.GONE
            binding!!.edtAnswerQ5.visibility = View.GONE
        }
    }
